...
```{r}
df <- homedf %>%
dplyr:: select(everything(),-Id,- PoolQC,-MiscFeature)
```
...
```{r}
df <- homedf %>% mutate( TotalLivingAreaSF = GrLivArea + BsmtFinSF1 + BsmtFinSF2)


```

```{r}
 summary(df$TotalLivingAreaSF)
```

```{r}
#Create new column total number of bathrooms in ground level & basement 
df <- homedf %>% mutate( TotNoBath = FullBath + HalfBath ,TotNoBsmtBath = BsmtFullBath +
BsmtHalfBath )
homedf
...
df<- homedf %>% mutate(AgeOfHouse = YrSold- YearBuilt)
summary( df$AgeOfHouse)
...
```{r}
df<- homedf %>% mutate(TotalNoBath = FullBath+HalfBath+BsmtFullBath+BsmtHalfBath)
table(df$TotalNoBath)
```


#check for zero variance predictors

We can reduce the dimension of the data by removing those columns for which varaince is zero, because zero variance columns have unique values. So those column doesn't impact the output at all and can be safely removed from the dataset.
...
There are 20 near zero variance variables in the training dataset. 

```{r}
names(df[nzv_cols])
```

#remove the near zero variance predictors from dataset

```{r}
if(length(nzv_cols) > 0) 
    hdf <- df[, -nzv_cols] # storing to a new data frame
```
...
```{r}
ggplot(hdf, aes(x = Neighborhood, y = SalePrice)) + 
...
Neighborhood and age of house

```{r}
ggplot(hdf, aes( y=AgeOfHouse, x=Neighborhood,fill=YearBuilt)) + 
...
```{r}
summary(hdf$GrLivArea)
```
The smallest house is 334 sqft and the biggest is 5642sqft

```{r}
ggplot(hdf, aes( y=SalePrice, x= GrLivArea )) + 
geom_point()+
   scale_y_continuous(labels = dollar)
```
...
```{r}
ggplot(hdf, aes( y=SalePrice, x= log(GrLivArea))) + 
geom_point()+
  geom_smooth()+
   scale_y_continuous(labels = dollar)
```
...
```{r,echo=FALSE}
ggplot(hdf, aes(x = LotArea , y = GrLivArea)) +
  geom_line() +
  labs(x = "Lot Area") +
...
```{r}
ggplot(hdf, aes(LotArea, SalePrice)) + 
...
```{r}
ggplot(hdf, aes(GrLivArea, SalePrice)) + 
  geom_line(color="blue", size=2) +
  scale_y_continuous(" Sale Price", labels = dollar) +
...
```{r}
ggplot(hdf, aes(x = GrLivArea, y = SalePrice)) + 
  geom_point() +
   scale_y_continuous(labels = dollar)+ 
...
```{r}
ggplot(hdf, aes(x = log(LotFrontage), y = SalePrice)) + 
  geom_point() +
   scale_y_continuous(labels = dollar)+ 
  geom_smooth(method = "lm")
```
...
ggplot(hdf, aes(x = LotArea, y = SalePrice)) + 
...
```{r}
ggplot(hdf, aes(x = log(LotArea), y = SalePrice)) + 
  geom_point(alpha = 0.2) + 
  geom_smooth(method = "lm") + 
  scale_y_continuous(labels = dollar)
```
...
```{r}
hdf$Street <-as.factor(homedf$Street)
table(hdf$Street)
...
index <- which(hdf$LotShape == "IR2" | hdf$LotShape == "IR3")
hdf[index, 'LotShape'] <- "Mod+ IR"
...
```
Mean sale price is high on lot shape moderately irregular or irregular. 
...
It seems like properties on irregular,raised and hillside land sale pice mean value is more than the regular shaped lots.It seemslike the cost of construction is high for building a house in land that's not flat or regular shape.
...
ggplot(hdf, aes(fill=LotConfig, y=SalePrice, x=LotConfig)) + 
geom_bar(position = "dodge", stat="identity")+
   scale_y_continuous(labels = dollar)
...
```{r}
#Combine Duplex and Twnhs into one level 
index <- which(hdf$BldgType == "Duplex" | hdf$BldgType == "Twnhs")
hdf[index, 'BldgType'] <- "Duplex+ Twnhs"
# New median sale price by lot shape
hdf %>% 
  group_by(BldgType) %>% 
  dplyr::summarise(n = n(), 
...
```{r}
ggplot(hdf, aes(x = BldgType)) +
  geom_histogram( stat= "count",binwidth = 15,fill = "orange",color = "grey" ) +
...
```{r}
ggplot(hdf, aes(x = BldgType, y = SalePrice, color = HouseStyle)) + geom_point()+
   scale_y_continuous(labels = dollar)+
   theme(axis.text.x = element_text(angle = 90))+
...
```{r}
ggplot(hdf, aes( x=SalePrice,y=BldgType,color=OverallQual)) + 
...
```{r}
 ggplot(hdf, aes(x = YearBuilt, y = SalePrice, color = YearRemodAdd)) + geom_point()+
   scale_y_continuous(labels = dollar)
...
```{r}
ggplot(hdf,aes(x = SalePrice,color=ExterQual))+
...
```{r}
ggplot(hdf,aes(x = SalePrice, color= ExterCond))+
  geom_histogram(biwidth = 1000)+
  scale_x_continuous(labels = dollar)+
   theme(axis.text.x = element_text(angle = 90))+
...
```{r}
hdf %>% count(hdf$BsmtQual,BsmtFullBath,BsmtHalfBath)

```


```{r}
 ggplot(hdf, aes(x = log(BsmtFinSF1), y = SalePrice, color = BsmtQual)) + 
  geom_point()+
...
```{r}
 ggplot(hdf, aes(x = BsmtQual, y = SalePrice, color=BsmtHalfBath)) + geom_point()+
   scale_y_continuous(labels = dollar)+
...
```{r}
hdf %>% count(homedf$CentralAir)


```

```{r}
hdf %>% count(homedf$HeatingQC)
```
...
```{r}
 ggplot(hdf, aes(x = HeatingQC, y = SalePrice)) + geom_bar(stat = "identity")+
   scale_y_continuous(labels = dollar)
```
...
```{r}
hdf %>% count(hdf$HouseStyle)
```

```{r}
hdf %>% count(hdf$X2ndFlrSF==0)
```
...
```{r}
hdf %>% count(homedf$GrLivArea,BedroomAbvGr)
```


```{r}
 ggplot(hdf, aes( BedroomAbvGr,GrLivArea,color = BedroomAbvGr)) + geom_line()
...
#   scale_y_continuous(labels = dollar)+
ggplot(hdf, aes(x = BedroomAbvGr, y = SalePrice, fill = HalfBath)) +
  geom_boxplot() +
   scale_y_continuous(labels = dollar)+
  facet_wrap(~FullBath)
...
Newer homes sell for higher price.
...
train_index <- createDataPartition(y = hdf$SalePrice, p = 0.7, list = FALSE , times = 1)
# index train and test data
train <- hdf[train_index, ]
test <- hdf[-train_index, ]
...
```{r}
model <- lm(SalePrice ~ OverallQual+ LotConfig + Neighborhood + BldgType  + TotalBsmtSF + GrLivArea + FullBath + HalfBath  + KitchenQual + GarageArea + WoodDeckSF+ AgeOfHouse + YearRemodAdd , data = train)
summary(model)
```
...
```{r}
cor(train$SalePrice,train$AgeOfHouse)
cor(train$SalePrice,train$YearRemodAdd)
cor(train$AgeOfHouse,train$YearRemodAdd)
...
```{r}
modelrf1 <- randomForest(SalePrice~ . , data = train,ntree = 300 )
print(modelrf1)
summary(modelrf1)
...
```{r}
min_depth <- min_depth_distribution(RF1)
...
```{r}
variable_importance <- measure_importance(RF1)
...
```{r}
interactions <- min_depth_interactions(RF1, vars)
...
```{r}
plot_predict_interaction(RF1, train, "GrLivArea", "OverallQual")
```
